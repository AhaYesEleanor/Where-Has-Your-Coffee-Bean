...
def validate_model(unfitted_model, cv_folds=8):
    '''
    ...
    '''
    logloss = -1 * cross_val_score(unfitted_model, X_train, y_train, cv=cv_folds, scoring='neg_log_loss').mean()
    acc = cross_val_score(unfitted_model, X_train, y_train, cv=cv_folds, scoring='accuracy').mean()
    return logloss, acc
...
model_options = ('rfc', 'svc', 'sgd', 'gnc', 'dtc')
...
def get_best_pipeline(hyperopt_output):
    ...
    if model_type == 'rfc':
        ...
        class_weight = class_weight_options[model_params['dtc_class_weight']]
        model = DecisionTreeClassifier(**model_params)
...
def plot_data_by_feats(x_feat, y_feat, X, class_dict, feat_plot_filename):
    '''
    Takes in two features of interest, a full set of X data, and a dictionary relating class labels to class id numbers
